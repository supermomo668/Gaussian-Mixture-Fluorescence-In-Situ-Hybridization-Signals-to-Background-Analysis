...
def collect_channel_pixel(summary_df, ch="L532S", crop = .25):
    channel_pixels = []
    ch_df = summary_df.loc[summary_df["Channel"]==ch]
    for ch_path in ch_df["Path"]:
        im = cv2.imread(str(ch_path), -1)
        start_ = int((1-np.sqrt(1-crop))*len(im))
        end_ = len(im)-start_
        channel_pixels.append(im[start_:end_, start_end_].ravel())
