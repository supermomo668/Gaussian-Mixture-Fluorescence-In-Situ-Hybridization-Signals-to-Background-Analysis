...
def summarize_snr_for_directory(montage_path=False, img_dir=WORKING_DIR, num_sample=False, fovs:list=[]):
    '''
...
def summarize_snr_for_directory_all_channels(cyc_dir: Path , num_sample: int, fovs:list):
    ...
    if num_channel >= 3 :
        ...
        for montage_path in montage_paths:
            sum_dfs.append(summarize_snr_for_directory(montage_path, img_dir=cyc_dir/"AcqData", 
                            num_sample= num_sample, fovs = fovs))
...
def summarize_snr_for_all(exp_dir: Path, num_sample: int, fovs:list):
    cyc_folders = []
    ...
    for cyc_folder in cyc_folders:
        sum_dfs.append(summarize_snr_for_directory_all_channels(cyc_folder, num_sample, fovs))
    summary_df = pd.concat(sum_dfs, axis=0)
    summary_df[["Image"]+[c for c in summary_df.columns if c not in ["Image"]]]
    return summary_df

def main(exp_dir: Path, img_dir: Path, settings: dict):
    ...
    if not single_cycle:  # provided experiment path
        ...
        summarize_snr_for_all(exp_dir, num_sample=num_sample, fovs=fovs).to_csv(exp_dir/"SNR-summary_all.csv")
    else:
        acq_dir = exp_dir
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_path.parent/"SNRA-settings.ini"):
        ...
        if "User Settings" not in config.sections(): settings = config["Default"]
        else: settings = config["User Settings"]

    if len(args.exp_path) == 0:
        print("Using default experimental path")
        args.exp_path = file_path.parent
        single_cycle = False
    if "AcqData" in os.listdir(args.exp_path):
        single_cycle = True
    main(exp_dir= Path(args.exp_path), img_dir = Path(args.image_path), settings = settings)